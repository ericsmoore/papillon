...
def build(obfuscate=False):
    for s in scenes.rglob('*.md'):
        basename = os.path.basename(s)
        if basename == 'index.md' or not obfuscate:
    ...
    for s in scenes.rglob('*.md'):
        ...
        def replace_link(m):
            ...
            if basename in scene_map:
                target = Path(scene_map[basename]).stem
...
def serve():
    Handler = partial(
        http.server.SimpleHTTPRequestHandler, directory=site_output
    )
